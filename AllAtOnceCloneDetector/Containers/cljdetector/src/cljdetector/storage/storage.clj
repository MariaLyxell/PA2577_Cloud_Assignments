...
;; (def collnames ["files"  "chunks" "candidates" "clones"])        ;; Replaced this line with the line below.
...
;; Added this function 
(defn addUppdate! [time message]
  (let [conn (mg/connect {:host hostname})        
      db (mg/get-db conn dbname)
      collname "statusupdates"]
  (mc/insert db collname {:message message :timestamp time})))
